...
export class Tornado implements IUpdatable {
    ...
    constructor(world: World, position: THREE.Vector3) {
        ...
        const cylinderShape = new CANNON.Cylinder(this.tornadoRadius, this.tornadoRadius, this.tornadoHeight, 8);
        this.body = new CANNON.Body({
            ...
            collisionFilterMask: this.world.physicsManager.CollisionGroups.Default | this.world.physicsManager.CollisionGroups.Characters | this.world.physicsManager.CollisionGroups.Vehicles
        });
    ...
    public update(deltaTime: number): void {
        ...
        for (let i = 0; i < this.particleCount; i++) {
            ...
            this.particlePositions[i3_1] += this.particleVelocities[i3_1] * deltaTime * 10;
        ...
        // Movement
        const direction = new THREE.Vector3().subVectors(this.targetPosition, this.particles.position);
        if (direction.length() < 5) { // If close to target, set a new one
            this.setRandomTargetPosition();
        }
        ...
        this.world.physicsManager.physicsWorld.bodies.forEach(otherBody => {
            if (otherBody.mass > 0 && otherBody !== this.body) { // Only affect dynamic bodies
                ...
                if (distance < affectedRadius) {
                    ...
                    // Swirling force
                    const dirToBody = new CANNON.Vec3().copy(otherBody.position).vsub(tornadoCenter);
                    dirToBody.y = 0; // Only horizontal component
                    dirToBody.normalize();

                    const swirlDir = new CANNON.Vec3(-dirToBody.z, 0, dirToBody.x); // Perpendicular for swirl
                    forceVector.vadd(swirlDir.scale(swirlForce * (1 - distance / affectedRadius)), forceVector);
