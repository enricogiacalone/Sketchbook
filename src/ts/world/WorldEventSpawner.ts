...
export class WorldEventSpawner {
  ...
  public update(timeStep: number): void {
    ...
    for (let i = this.meteorites.length - 1; i >= 0; i--) {
      ...
      if (meteorite.isRemoved) {
        // Meteorite self-removes on collision
        this.meteorites.splice(i, 1);
      } else if (meteorite.body.position.y < -100) {
        // Remove if fallen far below ground
    ...
    for (let i = this.atomicExplosions.length - 1; i >= 0; i--) {
      ...
      if (atomicExplosion.state === "done") {
        // Assuming AtomicExplosion has a public 'state' property
    ...
    for (let i = this.ufos.length - 1; i >= 0; i--) {
      ...
      if (ufo.isRemoved) {
        // UFO self-removes after lifetime
