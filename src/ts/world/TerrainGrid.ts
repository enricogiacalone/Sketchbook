...
export class TerrainGrid {
    ...
    private getGridCoordinates(x: number, z: number): { gx: number; gz: number } | null {
        const gx = Math.floor((x + this.halfSize) / this.resolution);
        const gz = Math.floor((z + this.halfSize) / this.resolution);

        if (gx >= 0 && gx < this.grid.length && gz >= 0 && gz < this.grid[0].length) {
            return { gx, gz };
        }
        return null;
    }
    ...
    public isOccupied(x: number, z: number, checkType?: TerrainCellType): boolean {
        const coords = this.getGridCoordinates(x, z);
        if (coords) {
    ...
    public markArea(x1: number, z1: number, x2: number, z2: number, type: TerrainCellType): void {
        const startCoords = this.getGridCoordinates(Math.min(x1, x2), Math.min(z1, z2));
        const endCoords = this.getGridCoordinates(Math.max(x1, x2), Math.max(z1, z2));
