...
export class Car extends Vehicle implements IControllable {
    ...
    constructor(gltf: any) {
        ...
        this.physicsPreStep = this.physicsPreStepCar;
    ...
    public update(timeStep: number): void {
        ...
        if (this.shiftTimer > 0) {
            ...
            if (this.actions.reverse.isPressed) {
                const powerFactor =
                    (gearsMaxSpeeds['R'] - this.speed) /
                    Math.abs(gearsMaxSpeeds['R']);
                const force =
                    (engineForce / this.gear) * Math.abs(powerFactor) ** 1;
        ...
        if (this.steeringWheel !== undefined)
            this.steeringWheel.rotation.z =
                -this.steeringSimulator.position * 2;
    ...
    public physicsPreStepCar(body: CANNON.Body): void {
        // Constants
        ...
        let airSpinInfluence = THREE.MathUtils.clamp(
            this.airSpinTimer / 2,
            0,
            1
        );
        ...
        if (this.actions.right.isPressed) {
            let steering = Math.min(
                -maxSteerVal / speedFactor,
                -driftCorrection
            );
