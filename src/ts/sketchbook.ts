...
setInterval(() => {
    if (myPlayerId && world.player) { // Assuming world.player exists and has position/quaternion
        const playerPosition = world.player.position;
        const playerQuaternion = world.player.quaternion;
        const playerAnimation = world.player.charState ? world.player.charState.constructor.name.toLowerCase() : "idle"; // Get current animation state
