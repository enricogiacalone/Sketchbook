...
export class Character extends THREE.Object3D implements IWorldEntity {
    ...
    public setPosition(x: number, y: number, z: number): void {
        if (this.physicsEnabled) {
            this.characterCapsule.body.previousPosition = new CANNON.Vec3(
                x,
                y,
                z
            );
    ...
    public rotateModel(): void {
        ...
        this.tiltContainer.position.setY(
            Math.cos(
                Math.abs(this.angularVelocity * 2.3 * this.velocity.length())
            ) /
                2 -
                0.5
        );
    ...
    public findVehicleToEnter(wantsToDrive: boolean): void {
        ...
        if (vehicleFinder.closestObject !== undefined) {
            ...
            // Find best seat
            let seatFinder = new ClosestObjectFinder<VehicleSeat>(
                this.position
            );
            ...
            if (seatFinder.closestObject !== undefined) {
                ...
                if (entryPointFinder.closestObject !== undefined) {
                    vehicleEntryInstance.entryPoint =
                        entryPointFinder.closestObject;
    ...
    public physicsPreStep(body: CANNON.Body, character: Character): void {
        ...
        if (character.rayHasHit) {
            if (character.raycastBox.visible) {
                character.raycastBox.position.x =
                    character.rayResult.hitPointWorld.x;
                character.raycastBox.position.y =
                    character.rayResult.hitPointWorld.y;
                character.raycastBox.position.z =
